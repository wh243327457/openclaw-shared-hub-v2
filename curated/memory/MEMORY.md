...
- `foundation/path-portability` 已升格为共享 skill，用于约束运行时代码通过 `resolve_shared_root.py` 解析宿主根，禁止硬编码 `/home/vany/...`；`manifest.yaml: deployment` 段同步建立解析顺序、跨机器搬运最小保留集与 git 忽略规则
