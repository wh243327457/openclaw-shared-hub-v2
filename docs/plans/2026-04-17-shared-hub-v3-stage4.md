...
**Architecture:** stage4 不改造现有 `<shared-root>`，也不把 `compat/`、`memory/`、`skills/` 重新带回 v3。新增一个仅由 Hermes 触发的 migration executor，读取 v2 `manifest.yaml + AGENTS.md + curated/memory/`，经过 preflight 后把稳定内容复制到 v3 `truth/memory/`，并写入独立的 `truth/migration-logs/` 审计记录与 `truth/memory/MEMORY.md` 的受托管迁移索引块。
...
**Objective:** 让 verifier 不依赖真实本机 `<shared-root>`，可在 staging repo 内自测。
